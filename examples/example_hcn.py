...
spectral_orders = list(set(np.arange(28))-set([18,19,20]))
dataset = load_carmenes_data(folder='/users/ricolandman/Documents/Research/OH_exoplanets/Data/Wasp76_NIR/', spectral_orders=spectral_orders)
#dataset = load_carmenes_data(folder='/users/ricolandman/Documents/Research/OH_exoplanets/Data/Wasp76_NIR/', skip_exposures=[0, 44, 43, 42, 41, 40, 39, 38, 37], spectral_orders=[23,24,25,26,27])

template= 1-fits.open('/users/ricolandman//Downloads/syn_trans_carmenes_HCN_wasp76b_terminator.fits')[0].data
template = highpass_gaussian(template, sigma=2000)
template_wl= fits.open('/users/ricolandman/Downloads/wave_carmenes_NIR.fits')[0].data*1e4
...
pipeline.add_module( FlagAbsorptionEmissionModule(flux_lower_limit=0.4, flux_upper_limit=1.05, relative_to_continuum=True) )
pipeline.add_module( PolynomialContinuumRemovalModule(poly_order=3) )
pipeline.add_module( OutlierFlaggingModule(sigma=5) )

pipeline.add_module( SysRemModule(number_of_modes=7, mode='subtract', savename='cleaned') )

pipeline.add_module( CrossCorrelationModule(template = template, template_wl = template_wl, rv_range=250, drv=1.3, error_weighted=False))
...
pipeline.run(dataset, num_workers=8) 
pipeline.summary()
